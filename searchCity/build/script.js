...
req.onreadystatechange = function () {
	if (this.readyState == 4 && this.status == 200) {
		...
		cities.push.apply(cities, _toConsumableArray(answ));
		// console.log(cities);
	}
};
...
function buildList(list, text, reg) {
	...
	var html = list.map(function (item) {
		var city = item.city.replace(reg, '<span class="hl">' + text + '</span>');
		var state = item.state.replace(reg, '<span class="hl">' + text + '</span>');
		var li = '<li><span class="name">';
		li += city + ', ' + state + '</span>';
...
function searchCity() {
	...
	var list = cities.filter(function (city) {
		return city.city.search(reg) != -1 || city.state.search(reg) != -1;
	});
